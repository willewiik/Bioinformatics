...
plot_bipartite_network <- function(edge_matrix, title = "Bipartite Network") {
  ...
  #print(length(unique(comms)))
  mems <- membership(comms)
  comm_col <- rainbow(length(unique(comms)))[mems]
  #print(length(comm_col))
  ...
  legend("bottomright", legend = paste("Community", 1:length(unique(comms))),
         fill = unique(comm_col))
